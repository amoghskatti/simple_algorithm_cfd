...
int main()
{
    ...
    while(step<=500)
    {
        ...
        while(error>0.0000001)
        {
            ...
            while(errorT>0.0000001)
            {
                ...
                for(i=1;i<=grid-2;i++)
                {
                    ...
                    {
                        ap[i][j] =  (1.0/dt)
                                    +(u[i+1][j]+u[i][j])/(4.0*dx) - (u[i-1][j]+u[i][j])/(4.0*dx)
                                    +(v[i+1][j]+v[i][j])/(4.0*dy) - (v[i+1][j-1]+v[i][j-1])/(4.0*dy)
                                    + 2.0/(Re*dx*dx) + 2.0/(Re*dy*dy);
                        ae[i][j] = -(u[i+1][j]+u[i][j])/(4.0*dx) + 1.0/(Re*dx*dx);
                        aw[i][j] = (u[i-1][j]+u[i][j])/(4.0*dx) + 1.0/(Re*dx*dx);
                        an[i][j] = -(v[i+1][j]+v[i][j])/(4.0*dy) + 1.0/(Re*dy*dy);
                        as[i][j] = (v[i+1][j-1]+v[i][j-1])/(4.0*dy) + 1.0/(Re*dy*dy);
                        A[i][j] = 1.0/dx;
                        du[i][j] = A[i][j]/ap[i][j];
                        B[i][j] = u[i][j]/dt;
                    }
                ...
                {
                    un[i][0] = -un[i][1];
                    un[i][grid] = 2.0*sin(w*step*dt)-un[i][grid-1];
                }
            ...
            while(errorT>0.0000001)
            {
                ...
                for(i=1;i<=grid-1;i++)
                {
                    ...
                    {
                        ap[i][j] =  (1.0/dt)
                                    +(u[i][j+1]+u[i][j])/(4.0*dx) - (u[i-1][j+1]+u[i-1][j])/(4.0*dx)
                                    +(v[i][j+1]+v[i][j])/(4.0*dx) - (v[i][j-1]+v[i][j])/(4.0*dx)
                                    + 2.0/(Re*dx*dx) + 2.0/(Re*dy*dy);
                        ae[i][j] = -(u[i][j+1]+u[i][j])/(4.0*dx) + 1.0/(Re*dx*dx);
                        aw[i][j] = (u[i-1][j+1]+u[i-1][j])/(4.0*dx) + 1.0/(Re*dx*dx);
                        an[i][j] = -(v[i][j+1]+v[i][j])/(4.0*dy) + 1.0/(Re*dy*dy);
                        as[i][j] = (v[i][j-1]+v[i][j])/(4.0*dy) + 1.0/(Re*dy*dy);
                        A[i][j] = 1.0/dy;
                        dv[i][j] = A[i][j]/ap[i][j];
                        B[i][j] = v[i][j]/dt;
                    }
            ...
            {
                u0[i][0] = -u0[i][1];
                u0[i][grid] = 2.0*sin(w*step*dt)-u0[i][grid-1];
            }
        ...
        // OUTPUT DATA
        FILE *fout2, *fout3;
        fout2 = fopen("oscillating_UVP.plt","a+t");
        fout3 = fopen("oscillating_Central_U.plt","a+t");
